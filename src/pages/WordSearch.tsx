...
const WordSearch = () => {
  ...
  // Strike-through line component
  const StrikeLine = ({ placement }: { placement: WordPlacement }) => {
    const { startPos, direction, length } = placement;
    
    // Calculate line length (in rem units)
    const lineLength = length * 2; // 2rem per cell
    
    // Calculate rotation angle based on direction
    let rotation = 0;
    if (direction.x === 1 && direction.y === 0) rotation = 0;        // right
    else if (direction.x === 0 && direction.y === 1) rotation = 90;  // down
    else if (direction.x === 0 && direction.y === -1) rotation = 90; // up
    else if (direction.x === 1 && direction.y === 1) rotation = 45;  // diagonal right-down
    else if (direction.x === 1 && direction.y === -1) rotation = -45; // diagonal right-up

    return (
      <div
        className={`absolute transition-opacity ${showAnswers ? 'opacity-100' : 'opacity-0'}`}
        style={{
          ...
          transform: `translateY(1rem) rotate(${rotation}deg)`,
          zIndex: 10
        }}
      />
    );
